...
require_once('../connection.php');
    ...
    if(isset($_FILES['excelFile']) && !empty($_FILES['excelFile']['tmp_name'])){
        ...
        for($i=0;$i<$highestRow;$i++){
            ...
            $teacher_name = $get_roll[0];
            
            $get_roll = explode("@",$getsheet[$i][0]);  
            $teacher_username = $get_roll[0];
            
            $get_roll = explode("@",$getsheet[$i][2]);  
            $teacher_email = $get_roll[0];
            
            $get_roll = explode("@",$getsheet[$i][3]);  
            $teacher_number = $get_roll[0];

            $get_roll = explode("@",$getsheet[$i][1]);  
            $teacher_password = $get_roll[0];
            $teacher_password = md5($teacher_password);
            // $teacher_name;
            $teacher_table_query = "INSERT INTO `teacher`( `teacher_name`) VALUES ('$teacher_name')";
      ...
      <?php require_once("./sidebar.php"); ?>
        <div class="dashboard-inner " id="main-bar">
            <div class="dashboard-inner-top">
                <h2>
                    All the Teachers are Uploaded Successfully
                </h2>
